...
Podczas logowania nastêpuje sprawdzenie czy u¿ytkownik istnieje w bazie i czy has³o jest prawid³owe. Je¿eli tak z bazy zostaje pobierana rola przypisana do u¿ytkownika oraz adres url zdjêcia je¿eli zosta³o ono wczeœniej przez niego dodane i generowany jest token. W przypadku niepowodzenia zwracany jest wyj¹tek braku autoryzacji. Przedstawia to listing \ref{list:req2}.
...
Logowanie jest konieczne, aby mieæ dostêp do pozosta³ych funkcjonalnoœci aplikacji. Podczas  zapytania ka¿dego do punktu koñcowego (za wyj¹tkiem logowania oraz rejestracji), sprawdzany jest token i w przypadku jego braku, lub niewa¿noœci zwracany jest kod 401, œwiadcz¹cy o braku autoryzacji. \\
...
Aplikacja posiada funkcjonalnoœæ wgrywania zdjêæ, które mo¿na nastêpnie wykorzystaæ jako avatar. Zdjêcie zostaje przypisane do u¿ytkownika, oraz wybrane jako zdjêcie g³ówne je¿eli jest to pierwsze zdjêcie tego u¿ytkownika. Kod odpowiadaj¹cy za wgrywanie zdjêcia znajduje siê na listingu \ref{list:req4}.
...
Zdjêcie zapisane zostaje na serwisie chmurowym o nazwie Cloudinary, a w bazie przechowywany jest jedynie link do niego.\\
\indent Zaimplementowano równie¿ mo¿liwoœæ podgl¹du profilu innych u¿ytkowników. Na podstawie przes³anej w zapytaniu nazwy u¿ytkownika z bazy pobierane s¹ informacje na jego temat wraz ze zdjêciem g³ównym je¿eli takowe posiada. Do znalezienia u¿ytkownika wykorzystano metodê SingleOrDefaultAsync, co oznacza ¿e w przypadku braku u¿ytkownika o podanej nazwie funkcja zg³asza wyj¹tek. Sposób pobierania danych na temat profilu przedstawia listing~\ref{list:req5}.
...
Usuwanie obiektu wykonywane jest na podstawie Id wys³anego w zapytaniu. Obiekt o wys³anym Id jest szukany w bazie, gdy takiego nie ma zg³aszany jest wyj¹tek. W przypadku powodzenia sezon zostaje usuniêty a zmiany zostaj¹ zapisane w bazie. W przypadku problemów z zapisem zg³aszany jest wyj¹tek. Omawian¹ sytuacjê przedstawia listing \ref{list:req9}.
...
Mecze mo¿na planowaæ dodaj¹c je do bazy ustawiaj¹c w³aœciwoœæ StartDate na datê w przysz³oœci, mo¿na dodawaæ równie¿ mecze które odby³y siê w przesz³oœci. Podczas dodawania meczu w pierwszej kolejnoœci sprawdzane jest na podstawie daty rozpoczêcia meczu oraz dywizji przes³anej w zapytaniu czy dla wybranej daty zosta³ ju¿ utworzony sezon wraz z dywizj¹, je¿eli nie zg³aszany jest wyj¹tek. Nastêpnie pobierane s¹ dane na temat dru¿yn w sezonie, przy pomocy których tworzone s¹ obiekty \textit{TeamMatch} dla obu dru¿yn. Zostaj¹ one dodane do bazy oraz zapisane. Je¿eli operacja zakoñczy siê sukcesem tworzony jest obiekt meczu. Kod znajduje siê na listingu \ref{list:req12}.
...
Aby dodaæ nowy rzut nale¿y go wys³aæ do punktu koñcowego ,,/incidentsHub'' który wczeœniej zosta³ skonfigurowany w klasie Startup, jak pokazuje listing \ref{list:req16}.
