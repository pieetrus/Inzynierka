...
Rozdzia≥ ten poúwiÍcono aby omÛwiÊ sposÛb implementacji prototypu aplikacji przy pomocy wymienionych rozdzia≥ wyøej technologii. ArchitekturÍ ca≥ej aplikacji ukazano na rys.~\ref{fig:webappschema}.
...
\section{Back-end}
System zosta≥ podzielony na warstwy tak jak na rys.~\ref{fig:architekturasystemu}. Strza≥ki symbolizujπ ich zaleønoúÊ od siebie. Zastosowanie tego wzorca pozwala na niezaleønoúÊ bazy danych, zewnÍtrznych serwisÛw oraz warstwy prezentacji(w tym przypadku jest to API do zewnÍtrznego serwera z warstwπ prezentacyjnπ) i ich szybkπ oraz wygodnπ wymianÍ w razie potrzeby bez ingerowania w wewnÍtrzne warstwy aplikacji.
...
Zawiera wszystkie klasy uøyte w bazie danych. Znajdujπ siÍ tu takøe enumy oraz logika z nimi zwiπzana jak np. metoda do wydobywania opisu z obiektu typu enum.

\begin{figure}[h!tp]
	\centering
	\includegraphics[width=0.2\linewidth]{rys05/domainTree.png}
	\caption{Pliki i foldery znajdujπce siÍ w warstwie domain}
...
Odpowiada za konfiguracje poszczegÛlnych obiektÛw bazodanowych oraz za po≥πczenie z bazπ danych. Oddzielenie tej warstwy od warstwy domain pozwala na szybkπ i wygodnπ zmianÍ dostarczyciela bazy danych bez potrzeby edytowania samych obiektÛw. Znajdujπ siÍ tu takøe migracje.

\begin{figure}[htp]
	\centering
	\includegraphics[width=0.3\linewidth]{rys05/persistanceTree.png}
	\caption{Pliki i foldery znajdujπce siÍ w warstwie persistence}
...
Zawiera ca≥π logikÍ aplikacji. Jest zaleøna od warstwy domain i nie ma zaleønoúci w innych warstwach lub projektach. Zawiera komendy oraz zapytania (commands/queries) ktÛre sπ obs≥ugiwane przez rozszerzenie MediatR patrz rys.~\ref{fig:divisionCQRS}.
\begin{figure}[h]
	\centering
...
Jednak z powodu duøej iloúci tabel i relacji w nie znalaz≥a siÍ na nim tabela \textit{Player} oraz \textit{AspNetUsers}. TabelÍ \textit{Player} wraz z jej relacjami przedstawia rys.~\ref{fig:playerdatabaseschema}, natomiast tabela \textit{AspNetUsers} znajduje siÍ na rys.~\ref{fig:userdatabaseschema}.
